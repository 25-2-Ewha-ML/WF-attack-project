# -*- coding: utf-8 -*-
"""RF/mon+unmon_features_1/open_world_binary/optuna.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11xH4ylnMENOEdRAEcBIoefkAwC_YQLFX
"""
...
MONITORED_PATH = 'data/preprocessed/monitored_features_1.csv'
df_monitored = pd.read_csv(MONITORED_PATH)

# Monitored 데이터는 모두 '1'로 설정 (Positive)
df_monitored['label'] = 1

# 1-2. Unmonitored 데이터 (label이 -1인 상태)
UNMONITORED_PATH = 'data/preprocessed/unmonitored_features_1.csv'
df_unmonitored = pd.read_csv(UNMONITORED_PATH)

# Unmonitored 데이터는 모두 '-1'로 설정 (Negative)
df_unmonitored['label'] = -1

# 1-3. 데이터 병합
df = pd.concat([df_monitored, df_unmonitored], axis=0, ignore_index=True)

print(f"전체 데이터 개수: {len(df)}")
print(f"Monitored(1) 개수: {len(df[df['label']==1])}")
print(f"Unmonitored(-1) 개수: {len(df[df['label']==-1])}")
